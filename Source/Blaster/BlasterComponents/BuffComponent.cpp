...
void UBuffComponent::ReplenishShield(float ShieldAmount, float ReplenishTime)
{
	...
	ShieldRepenishAmount +=ShieldAmount;
}
...
void UBuffComponent::ShieldRampUp(float DeltaTime)
{
	...
	ShieldRepenishAmount -= ReplenishThisFrame;

	if (ShieldRepenishAmount <= 0.f || Character->GetShield() >= Character->GetMaxShield())
	{
		bReplenishShield = false;
		ShieldRepenishAmount = 0.f;
	}
}
